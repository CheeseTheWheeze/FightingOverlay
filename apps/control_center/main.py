...
def build_diagnostics_payload(log_path: Path, cv2_version: str | None = None) -> str:
    ...
    sys_path_lines = "\n".join(f"  - {entry}" for entry in sys_path_head)
    return "\n".join(
        [
            f"Log path: {log_path}",
            ...
            f"OpenCV: {cv2_version or 'unavailable'}",
            f"LOCALAPPDATA: {os.environ.get('LOCALAPPDATA', '')}",
            f"APPDATA: {os.environ.get('APPDATA', '')}",
            f"USERPROFILE: {os.environ.get('USERPROFILE', '')}",
            "sys.path head:",
            sys_path_lines or "  (empty)",
        ]
    )


def log_startup_diagnostics(log_path: Path) -> None:
    logging.info("STARTUP platform=%s", platform.platform())
    logging.info("STARTUP python=%s", sys.version.replace("\n", " "))
    logging.info("STARTUP executable=%s", sys.executable)
    logging.info("STARTUP cwd=%s", os.getcwd())
    logging.info("STARTUP argv=%s", sys.argv)
    logging.info("STARTUP sys.path.head=%s", sys.path[:6])
    logging.info("STARTUP frozen=%s", getattr(sys, "frozen", False))
    logging.info("STARTUP tk=%s tcl=%s", tk.TkVersion, tk.TclVersion)
    logging.info(
        "STARTUP env.LOCALAPPDATA=%s env.APPDATA=%s env.USERPROFILE=%s",
        os.environ.get("LOCALAPPDATA"),
        os.environ.get("APPDATA"),
        os.environ.get("USERPROFILE"),
    )
    logging.info("STARTUP log_path=%s", log_path)


def format_traceback_snippet(exc: BaseException, limit_lines: int = 16) -> str:
    lines = traceback.format_exception(type(exc), exc, exc.__traceback__)
    snippet = "".join(lines).strip().splitlines()
    return "\n".join(snippet[:limit_lines]) if snippet else "No traceback available."


def show_startup_messagebox(title: str, message: str, details: str, log_path: Path) -> None:
    body = f"{message}\n\nLog: {log_path}\n\nDetails:\n{details}"
    try:
        root = Tk()
        root.withdraw()
        ...
        logging.exception("Failed to show startup messagebox")


def show_fatal_error_ui(root: Tk, title: str, message: str, details: str, log_path: Path) -> None:
    ...
    try:
        ...
        pass
    style.configure("Error.TFrame", background="#1c1f24")
    style.configure("Error.TLabel", background="#1c1f24", foreground="#e7e9ee")
    style.configure("ErrorHeader.TLabel", background="#1c1f24", foreground="#ff5d5d", font=("Segoe UI", 14, "bold"))

    frame = ttk.Frame(root, padding=16, style="Error.TFrame")
    frame.grid(row=0, column=0, sticky="nsew")
    root.rowconfigure(0, weight=1)
    root.columnconfigure(0, weight=1)

    ttk.Label(frame, text=title, style="ErrorHeader.TLabel").grid(row=0, column=0, sticky="w")
    ttk.Label(frame, text=message, style="Error.TLabel", wraplength=760).grid(
        row=1, column=0, sticky="w", pady=(8, 12)
    )
    ttk.Label(frame, text=f"Log file: {log_path}", style="Error.TLabel").grid(
        row=2, column=0, sticky="w", pady=(0, 8)
    )

    text_frame = ttk.Frame(frame, style="Error.TFrame")
    text_frame.grid(row=3, column=0, sticky="nsew")
    text_frame.columnconfigure(0, weight=1)
    text_frame.rowconfigure(0, weight=1)
    frame.rowconfigure(3, weight=1)

    detail_box = Text(text_frame, height=12, bg="#0f1216", fg="#e7e9ee", relief="flat")
    detail_box.insert("1.0", details)
    detail_box.configure(state="disabled")
    detail_box.grid(row=0, column=0, sticky="nsew")
    scroll = ttk.Scrollbar(text_frame, command=detail_box.yview)
    scroll.grid(row=0, column=1, sticky="ns")
    detail_box.configure(yscrollcommand=scroll.set)

    actions = ttk.Frame(frame, style="Error.TFrame")
    ...
    ttk.Button(actions, text="Open Logs Folder", command=on_open_logs).grid(row=0, column=0, sticky="ew", padx=4)
    ttk.Button(actions, text="Copy Details", command=on_copy_details).grid(row=0, column=1, sticky="ew", padx=4)
    ttk.Button(actions, text="Exit", command=root.destroy).grid(row=0, column=2, sticky="ew", padx=4)


def enable_debug_console(log_path: Path) -> None:
    logging.getLogger().setLevel(logging.DEBUG)
    if os.name != "nt" or not getattr(sys, "frozen", False):
        logging.info("Debug console requested (non-Windows or not frozen).")
        return
    try:
        import ctypes
        ...
        logging.info("Debug console attached. Log path: %s", log_path)
    except Exception:
        logging.exception("Failed to attach debug console")


def apply_tk_compatibility_toggles(root: Tk) -> None:
    """Best-effort Tk tweaks for Windows 11 / virtualized GPU environments."""
    if os.name != "nt":
        return
    try:
        root.tk.call("tk", "useinputmethods", 0)
    except tk.TclError:
        logging.debug("Tk input method toggle not supported.")
    try:
        root.option_add("*tearOff", False)
    except tk.TclError:
        logging.debug("Tk tearOff toggle not supported.")


def check_opencv(show_dialog: bool, log_path: Path) -> tuple[bool, str | None]:
    try:
        cv2 = get_cv2()
    except Exception as exc:
        details = f"{type(exc).__name__}: {exc}"
        message = (
            "OpenCV (cv2) failed to import. The Control Center cannot start without it.\n"
            "Reinstall FightingOverlay using the latest release or contact support."
        )
        logging.exception("OpenCV import failed: %s", details)
        if show_dialog and os.environ.get("FIGHTINGOVERLAY_SUPPRESS_UI") != "1":
            show_startup_messagebox("Missing OpenCV", message, details, log_path)
...
def main() -> int:
    ...
    parser.add_argument(
        "--debug-console",
        action="store_true",
        help="Enable debug console/diagnostics (best-effort on Windows builds).",
    )
    ...
    if args.debug_console:
        enable_debug_console(log_path)
    log_startup_diagnostics(log_path)
    opencv_ok, cv2_version = check_opencv(show_dialog=not args.test_mode, log_path=log_path)
    if not opencv_ok:
        return 1
    cv2 = get_cv2()
    ...
    root = Tk()
    fatal_ui_shown = {"value": False}

    def _handle_fatal_exception(title: str, message: str, exc: BaseException) -> None:
    ...
    def _report_callback_exception(exc_type: type[BaseException], exc: BaseException, tb: object) -> None:
        ...
        show_fatal_error_ui(
            ...
            "An unexpected error occurred while the UI was running.",
            details,
            log_path,
        )
    ...
    try:
        ...
        diagnostics_text = ""
        if args.debug_console:
            diagnostics_tab = ttk.Frame(notebook, style="Card.TFrame")
        ...
        if diagnostics_tab is not None:
            diagnostics_container = ttk.Frame(diagnostics_tab, padding=12, style="Card.TFrame")
            diagnostics_container.grid(row=0, column=0, sticky="nsew")
            diagnostics_container.rowconfigure(1, weight=1)
            diagnostics_container.columnconfigure(0, weight=1)
            ttk.Label(diagnostics_container, text="Diagnostics", style="Subheader.TLabel").grid(
                row=0, column=0, sticky="w"
            )
            diagnostics_box = Text(
            ...
            diagnostics_box.insert("1.0", diagnostics_text)
            diagnostics_box.configure(state="disabled")
            diagnostics_box.grid(row=1, column=0, sticky="nsew", pady=(8, 0))
            diagnostics_scroll = ttk.Scrollbar(diagnostics_container, command=diagnostics_box.yview)
            diagnostics_scroll.grid(row=1, column=1, sticky="ns", pady=(8, 0))
            diagnostics_box.configure(yscrollcommand=diagnostics_scroll.set)
            diagnostics_actions = ttk.Frame(diagnostics_container)
            diagnostics_actions.grid(row=2, column=0, sticky="ew", pady=(8, 0))
            ttk.Button(
                diagnostics_actions,
                text="Copy Diagnostics",
                command=lambda: copy_to_clipboard(diagnostics_text),
        ...
        message = (
            "Control Center failed to start. The main UI could not be constructed.\n"
            "Check the log file for details, reinstall the app, or reset your settings."
        )
        _handle_fatal_exception("Control Center failed to start", message, exc)
        if args.ui_smoke_test:
            root.destroy()
            return 1
        root.mainloop()
